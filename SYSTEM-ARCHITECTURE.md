...
- **`src/pulse/PulseSetupManager.ts`** — Mutates `pulses.setups` from `PulseControlCommand` payloads (create/delete/rename setup, BPM, slot count, slot→bucket assign). On each hub project load (`ProjectGraphStore.useProject`), `ensureLoadedProjectPulseDefaults()` guarantees at least one setup and a valid `activePulseGuid` (creates a default setup when `setups` is empty; repairs a missing or stale `activePulseGuid` to the first setup).
