...
**Connector entity & editing.** A connection is a durable graph entity `connector` (`{ guid, kind, aGuid, bGuid, restLength, params }`), referenced symmetrically by both intents — single source of truth, no mirroring. It is stored via the opaque-entity path (`ProjectGraphStore.applyConnectorCommand` → `upsert/patch/remove`) and surfaces in `entities.connector` for controllers and in the renderer `config` payload (`connectors`) for the simulator. Removing either endpoint intent **cascades** the connector removal (`ProjectGraphStore.removeConnectorsReferencing`), so no stale link survives. surface-v2 edits links in the intent params overlay (`ConnectionsEditor`): list = connectors where `aGuid|bGuid` is the selected intent, Connect = `upsert`, kind/param = `patch`, trash = `remove`. Kinds/params are declared in `systemCapabilities.connectorTypes`. Connector **visuals are simulator-2d only** (`renderers/simulator-2d/src/connectors/` — rod=line, spring=zigzag, rope=sagging 5-point `RopeChain`); the motion itself is ordinary intent position updates every renderer already consumes, so `dmx-ts` needs no change.
