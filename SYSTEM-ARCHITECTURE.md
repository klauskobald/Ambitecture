...
- **`src/handlers/SystemProbeHandler.ts`** + **`src/probe/`** — Multi-purpose read-only query endpoint. A client sends **`system:probe`** `{ requestId, query, args? }`; the handler dispatches `query` through the **probe query registry** (`probe/ProbeQueryRegistry.ts`: `registerProbeQuery(name, fn)`, queries receive a read-only `ProbeContext` = `{ registry, projectManager, graphStore }`) and replies to the **requesting socket only** with **`system:probe:result`** `{ requestId, query, ok, data? | error? }`. Built-ins live in `probe/builtinProbeQueries.ts` (currently `connectedRenderers` → `{ type, guid }[]`, where `type` is the renderer kind — `dmx` \| `simulator-2d` \| `neewer` \| `screen` \| `starter-web-app` — read from the `type` field each renderer now sends in its `register` payload and stored in `ClientInfo.meta.type`). **Add capabilities by registering a query, never a new message type.**
