...
`surface-v2/src/core/help/` is a **reusable, domain-free** in-app help subsystem: a floating/host-attachable help panel plus a persistent draggable toggle icon. It takes **zero domain imports** — it knows nothing about intents, the hub, or the layout shell. Callers couple it to the app through two injection points wired in [`src/app/main.js`](modules/controllers/surface-v2/src/app/main.js): a **host registry** and a **conduit** (see also the controller-side reuse rule under [Obligatory Guidance For Coding Agents](#obligatory-guidance-for-coding-agents)).

- **`HelpManager.js`** — the singleton. Public API `{ registerHost, show, hide, setConduit }`. `show(key, { host?, onClose? })` looks up a topic and renders it; with `host` it attaches into a registered host element as a full-host card, otherwise it floats (draggable + resizable, geometry persisted). The persistent **❓ toggle icon** is always in the DOM (draggable when standalone, parked in the panel header `.help-panel__actions` when a panel is open); clicking it flips global help visibility. **Non-mandatory** topics are silently skipped when help is toggled off and are dismissable via the `×` button or **Esc** (which fires the caller's `onClose`); **mandatory** topics (e.g. an interaction prompt like `connectIntent`) always show and cannot be Esc/×-dismissed. A `generation` counter guards against an in-flight async `show` being superseded; `hide()` is idempotent and re-entry safe.
