...
A channel under `params.channels` may carry a **`function`** (named `FnCurve` curve) describing the hardware's output response. The driving renderer applies it on output to linearise perception — e.g. `var/fixtures/neewer.yml` sets `channels.brightness.function: quadratic` because Neewer lamps reach near-full perceived output by ~25% input, so the `neewer` renderer sends `v²` (see `NeewerLightBase.curveBrightness`). This is **hardware compensation only**: `simulator-2d` intentionally does **not** apply it, since after compensation the physical lamp matches the simulator's linear (intended-perceived) brightness. When a channel has no `function`, output is passed through linearly (no implicit default).
