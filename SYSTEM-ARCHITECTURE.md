...
#### Rollout — dual-stream
Renderer subscribe is `{ events, fixtureState }`. **Resolved** renderers (`dmx-ts`, `simulator-2d`) set `fixtureState: true`; their fixture classes read `snapshot.sample('light.color.xyY')`, `'master.brightness'`, `'target'`, … straight from the received caps (no local resolution), keeping only render-time work: strobe **blink gating** (`_isStrobeOn(nowSec)`), the moving head's `atan2`+`panUnwrap`, the DMX/canvas write. `dmx-ts` is headless and `fixtureState`-only (its `LayerIntentEngine` is deleted); `simulator-2d` *also* keeps `events` to draw intent **markers** but reads fixture **output** from caps. **Legacy** renderers (`neewer`, `screen`) still set `subscribe.events: true` and resolve locally with their `LayerIntentEngine` copy — the hub emits both streams during the transition. Once `neewer`/`screen` migrate, the renderer-facing `events` resolution and the remaining `LayerIntentEngine` copies are removed.
