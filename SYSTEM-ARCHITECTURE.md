...
- **`PhysicsIntentAdapter`** — the consumer. Builds bodies from active-scene intents (`mass`/`drag`/`position`, skipping `master`) and connectors from `ProjectManager.getConnectors()`; wakes the engine when any participating intent is moved by a non-physics source (drag/animation/force) and pins it briefly; clamps each committed position to the stage AABB in the engine's commit hook (a clamped axis zeroes that velocity component — collision stays out of the engine); and streams results as transient `runtime:update`s (`source: 'hub:physics'`). `FixtureStateManager`'s own tick then picks up the new effective positions.
