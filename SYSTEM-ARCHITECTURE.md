...
- **`src/pulse/PulseManager.ts`** — Hub-side pulse orchestration: selects an active pulse setup, ticks slots, and fires bucket actions via `action:trigger`. Broadcasts `hub:status` (`kind: 'pulse'`) with live `data: { bpm, slotIdx, slotsTotal, speed }` on each tick (`speed` is the setup’s tempo multiplier for tick spacing; musical `bpm` is unchanged). Does not mutate project YAML for setups.
...
- **performPulsePanel.js** — Pulse subpane (between Control and Animate): list pulse setups (name + BPM + **speed** stepper) with select (▶), slot meter from `hub:status` (beat period uses `bpm * speed`), and create. Subscribes to `pulsePlayRegistry` and `projectGraph` topic `pulses`.
...
- `setSetupBpm` — `{ setupGuid, bpm }`; updates YAML; when that setup is active, hub refreshes runner BPM from project (pending timer unchanged, per existing pulse policy)
...
Surface: reusable **`PulseTapButton`** (`edit/components/PulseTapButton.js`) in the perform **subnav** title bar (left of ☰ / tabs) and beside the BPM knob in the pulse edit pane; **Ctrl+T** fires a tap for the active (or first) setup via `sendPulseTap`. List rows show setup name + BPM (live BPM when active) and **speed** controls (− halves, + doubles, clamped `0.25`–`4`).

**`hub:status`** (`kind: 'pulse'`, hub → controllers): `{ setupGuid, status: 'started' | 'stopped', message: { text }, data: { bpm, slotIdx, slotsTotal, speed } }`. Emitted on pulse tick, start, stop, and as a one-shot snapshot on controller register when a pulse is already running.
...
- **Pulse panel** (`performPulsePanel.js`) — pulse setup list (name + BPM + speed) with select/create/edit; slot meter from `hub:status` (`kind: 'pulse'`); subscribes to `pulsePlayRegistry`.
...
Manages pulse setups and selection. Payload is a `PulseControlCommand` (`selectSetup`, `createSetup`, `deleteSetup`, `renameSetup`, `setSetupBpm`, `setSetupSpeed`, `setSetupSlotCount`, `setSetupMode`, `assignSlotBucket`, `setSlotActive`, `setSyncConfig`). Setup changes are pushed as `projectPatch` key `pulses`; `selectSetup` drives `PulseManager` and subsequent `hub:status` (`kind: 'pulse'`) ticks. Only slots with `active: true` dispatch bucket actions on tick.
