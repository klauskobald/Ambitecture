...
- **`src/handlers/SystemProbeHandler.ts`** + **`src/probe/`** — Multi-purpose read-only query endpoint. A client sends **`system:probe`** `{ requestId, query, args? }`; the handler dispatches `query` through the **probe query registry** (`probe/ProbeQueryRegistry.ts`: `registerProbeQuery(name, fn)`, queries receive a read-only `ProbeContext` = `{ registry, projectManager, graphStore }`) and replies to the **requesting socket only** with **`system:probe:result`** `{ requestId, query, ok, data? | error? }`. Built-ins live in `probe/builtinProbeQueries.ts`: `connectedRenderers` → `{ type, guid }[]`, where `type` is the renderer kind — `dmx` \| `simulator-2d` \| `neewer` \| `screen` \| `starter-web-app` — read from the `type` field each renderer now sends in its `register` payload and stored in `ClientInfo.meta.type`; and `availableFixtureProfiles` → every fixture profile on disk under `fixturesPath` as `{ key, name, class, icon?, instance?, ... }[]` (`ProjectManager.listAvailableFixtureProfiles` — `readdir` + per-file `Config` parse), where `key` is the filename stem a `FixtureInstance.fixture` references. This feeds the operator's "add a fixture" picker, since profiles are otherwise only loaded for fixtures already placed. **Add capabilities by registering a query, never a new message type.**
...
- **`src/RuntimeIntentStore.ts`** — Hub-authoritative runtime overlay for perform intents. Stores a **dot-path patch bag** on top of the **current** active-scene row (`getActiveSceneIntent`), so definition, scene YAML overlay, and runtime streams stay separate: after a scene switch, paths never touched by `runtime:command` come from the new scene; patched paths keep last runtime values until `runtimeMergeClear` / eviction. `processRuntimeUpdates()` updates that bag and emits normalized renderer events; `getEffectiveIntent(guid)` returns baseline plus overlay; `listRuntimeOverlayGuidsInActiveScene()` drives reset affordance; `evictMergeGuids()` and `clear()` invalidate selectively (scene reset, project reload).
...
- **`src/snapshot/SnapshotManager.ts`** — Captures live scene + multi-pulse + multi-animation runner state into project `snapshots[]` on `snapshot:capture`; recalls via `action:trigger` on companion `{ type: "snapshot", guid }` actions, honoring per-row `recall` toggles. Delegates selective pulse/animation stop/start to `PulseManager.recallSnapshotPulses` / `AnimationManager.recallSnapshotAnimations`.
- **`src/handlers/SnapshotCaptureHandler.ts`** — Handles controller `snapshot:capture` (`{ name, recall, guid? }`); hub reads live runners (controllers never author overlay payloads).
- **`src/pulse/PulseManager.ts`** — Hub-side pulse orchestration: one independent tick loop per running setup (`startSetup` / `stopSetup`), fires bucket actions via `action:trigger`. Broadcasts `hub:status` (`kind: 'pulse'`) per setup with live `data: { bpm, slotIdx, slotsTotal, speed }` on each tick (`speed` is the setup’s tempo multiplier for tick spacing; musical `bpm` is unchanged). Project `activePulseGuid` is the sync/tap focus target. Does not mutate project YAML for setups.
- **`src/pulse/PulseSetupManager.ts`** — Mutates `pulses.setups` from `PulseControlCommand` payloads (create/delete/rename setup, BPM, **speed**, slot count, slot→bucket assign). On each hub project load (`ProjectGraphStore.useProject`), `ensureLoadedProjectPulseDefaults()` guarantees at least one setup and a valid `activePulseGuid` (creates a default setup when `setups` is empty; repairs a missing or stale `activePulseGuid` to the first setup).
...
- **`src/handlers/PulseControlHandler.ts`** — Handles controller `pulse:control`; applies setup mutations, calls `PulseManager.startSetup` / `stopSetup` / `setBPM`, broadcasts `projectPatch` key `pulses`.
...
- **`src/animation/keyframeAnimator.ts`** — Keyframe animation runner. Config is read **only** from `definition.content` (required object). `content.length` (seconds, finite > 0) is required; at least **two** steps are stored, with the earliest pinned to `time: 0` and the latest to `time: length` (centisecond rounding). Runtime playback uses the same centisecond rounding for the length boundary and per-step times so the last pinned keyframe is never dropped when raw float seconds disagree slightly with rounded ms. Endpoint rows omit `args` when empty. Plays time-ordered steps against each GUID in `targetIntents` (legacy singular `targetIntent` / `intent` when the array is absent), dispatching the same patch to every target through `RuntimeUpdateDispatcher`, and supports live edit mode (Add uses `diffRecordsToPatch`). The only animation class with built-in edit support today; new classes plug in via `intents/registry.ts`-style registration.
...
- Sends `register` payload as `role: "renderer"` with `type` (the renderer kind, e.g. `dmx` / `simulator-2d` / `neewer` — stored hub-side in `ClientInfo.meta.type` and surfaced via the `connectedRenderers` probe), `guid`, `location`, and `boundingBox` (optional metadata until hub `config` is authoritative for spatial bounds).
...
**`renderers/simulator-2d/`** — Browser 2D canvas preview of zones and fixtures (top-down XZ). Registers `dmx_light_static`, `dmx_basic_static`, `dmx_moving_head_mini`, `derby_3c`, `neewer_basic`, and `screen`; the `screen` footprint shows the mixed RGB from the same sampling path as the `screen` renderer's `singlePixel` algorithm (the simulator does not show which algorithm is selected). Fixture profile **`params.strobe`** overrides `FIXTURE_DRAW.strobe` for both **`screen`** and **`dmx_light_static`** when present; strobe-off frames draw **black** (`#000`). `LayerIntentEngine` matches screen/dmx-ts: **`light.strobe`** spatial accumulation does not multiply by intent **`alpha`**. Stroke-only class glyphs live under `assets/` (`rgb_simple.svg`, `screen.svg`, `movinghead.svg`, `derby_3c.svg`) and are composited with `drawImage` after canvas fills. `FIXTURE_DRAW.lamp.radius` (meters) sets the lamp circle radius and the `screen` box half-extent so one value scales every fixture glyph.
...
- **Touch overlay** (`src/viewport/overlayCanvas.js`): Transparent canvas stacked on top of the simulator iframe. Handles pointer events for intent/fixture dragging, draws a finger-trail, intent radius circles, out-of-zone markers, and selection bubbles. Supports modality via **interaction policies** and an optional **SelectionManager**. In **Stage edit** (default edit policy, no `SelectionManager`), a second tap on empty canvas within the double-tap window branches on the **fixture lock** (`getEditFixturesUnlocked`): **locked** runs `Modal.pickChoice` (Light \| Master \| Target), then mirrors **surface-v1** edit-pane sequencing: `projectGraph.putIntentRecord`, append controller ref, `toggleSceneIntent`, `graph:command` intent upsert, **controller `intents` patch before** `saveProject` scenes and `sendSceneActivate` (so the hub’s follow-up `projectPatch` intents list already includes the new ref and `reconcileIntents` does not prune the row), then opens intent params; **unlocked** runs `StageEditPane._onDoubleTapCreateFixture` — `projectGraph.getZoneAt` resolves the renderer-assigned zone under the tap (warn+cancel if none), a `hubProbe.probe('availableFixtureProfiles')` picker shows `Name (class)`, then it **optimistically** inserts the fixture (`projectGraph.putFixtureRecord`, dedup name + zone-relative location + the probed `fixtureProfile`) — the hub excludes the sending controller from its own `graph:delta` (`publishGraphMutation`), so the create must show locally without an echo, exactly like intent-create — before sending the durable fixture-create `graph:command` (below) and opening the fixture editor. Delete is the mirror: `queueFixtureRemove` + optimistic `projectGraph.deleteFixture`. (`StageEditPane` → `stageOverlayCoordinator.setEditDoubleTapHandlers`).
...
- **Outbound queue** (`src/core/outboundQueue.js`): Rate-limited WebSocket send queue for minimal `graph:command` updates. Intent changes are sent as GUID-addressed patches/removals; fixture moves are sent by fixture GUID (`queueFixtureUpdate`, `position` only); fixture instance-param edits go by fixture GUID via `queueFixturePropertyUpdate` (durable dot-path patch of root fields + `params`); scene saves are converted to graph upserts/removes. Also exposes `sendBindingSubscribe` / `sendBindingSet` / `sendAnimationEdit`.
...
- **PropertyPanel** (`src/edit/PropertyPanel.js`): Generic property editor card. Renders descriptor-driven controls from `intentProperties` (`display`: scalar sliders, color, string modal/pills, **`pills`**, read-only **`vector3`**) for one or many selected entities, integrates with `InputAssignManager`'s inline pane, and routes value changes through the project graph (`durable` for committed edits, `runtime` for live drag). Resolution uses **`resolveIntentDescriptorUiKind()`** in `systemCapabilities.js` (`display` first; legacy `type` UI kinds still supported). In **stage edit**, the same panel is hosted by **`IntentParamsHost`** (`src/stage/IntentParamsHost.js`) with a footer row (**Copy**, **Delete**) wired through **`intentBulkActions.js`** (graph + `saveProject` sequencing aligned with the legacy edit pane). The same `PropertyPanel`/`PropertyControl` widgets are reused by the **fixture instance editor** (`FixtureParamsHost`) via an optional injected **write target** `{ read, update, remove, save }`; when omitted the controls use the default intent path (scene-overlay aware), so intent behaviour is unchanged.
...
- **performPulsePanel.js** — Pulse subpane (between Control and Animate): list pulse setups (name + BPM + **speed** stepper) with play/stop (▶ / ■ via `startSetup` / `stopSetup`; multiple setups may run at once), slot meter from `hub:status` (beat period uses `bpm * speed`), and create. Subscribes to `pulsePlayRegistry` and `projectGraph` topic `pulses`.
- **performSnapshotPanel.js** — Snapshot subpane (layout class `snapshot`): capture modal, snapshot list with recall-flag labels, Modify (Save = metadata only; Capture = re-capture live state with existing `guid`)/Delete, perform assign rows. Uses `snapshot:capture`, `graph:command` on `entityType: "snapshot"`, and `action:trigger` on companion snapshot actions. **Play** also calls **`setPerformIntentFilter(snapshotGuid)`** so the MIDI/plugin assign list filters to that GUID.
...
- **performIntentFilter.js** (`src/core/performIntentFilter.js`) — Shared perform-mode **filter GUID** (or `null`) used by Animate and plugin iframes (historically “intent filter”). **performIntentFilterChip** shows the funnel chip (intent or snapshot name) on **Animate** and **plugin** subpanes. Single-tap an intent on the perform overlay toggles the filter when **Animate** or an active **plugin** subpane is selected. Snapshot **Play** also sets the filter to the snapshot GUID (companion action GUID). **PluginPane** adds **`filter=<guid>`** to the plugin iframe URL when set and drops it when the filter is cleared.

**Plugin UI panels (headless controllers → surface-v2 Perform):**
...
- **Iframe `filter` query** — Optional **`filter=<guid>`** on the plugin page URL (appended by **PluginPane** from **`performIntentFilter`**). MIDI assign UI matches assignments whose targets include an **`intent`** or **`action`** with that GUID; the hub graph is unchanged.

**`controllers/midi-v1/`** — Headless MIDI controller: **`graph:init` / `graph:delta`** drive **`GraphReplica`** assignments → receivers; **`graph:command`** `patch` on its own controller guid persists assignment edits from the plugin UI. Runs a small **HTTP + WebSocket** server (`PluginServer`, static `ui/assign.html`) for the assign editor; **`register`** includes **`discovery`** built from **`PLUGIN_PUBLIC_HOST`** + **`PLUGIN_LISTEN_PORT`**. The iframe connects to **`/ws`** on that server for live state (trusted LAN). The assign UI reads **`filter`** from the page URL and shows only assignments that include an **`intent`** or **`action`** target with that GUID (client-side; full list still arrives over the socket). Create is enabled when a filter is set: intent filter → intent param target; action/snapshot filter → **`type: "action"`** target (one-shot). Assignment class **`noteOnOff`** applies `(velocity + offset) × scale / 127` on velocity-gated note-on when **`params.envelope`** is off or disabled (same raw scaling spirit as CC paths); matching note-off sends **0**. Optional **`env_ar`** envelope uses **50 ms** ticks: **`triggerVelocity * envelopeLevel + offset`** then **`× scale / 127`**, with linear attack and release in milliseconds until idle. Class **`noteOnOffToggle`** uses the same **`params`** shape and on/off fan-out paths as **`noteOnOff`**, but latches on each velocity-gated note-on tap and ignores MIDI note-off (release does not send **0**); a second note-on on the same note toggles off.

- **Targets** — **`TargetIntent`**: continuous `runtime:command` intent patches (curve + `intentValueAdapter`). **`TargetAction`**: one-shot `action:trigger` on rising edge of normalized **`> 0`** (ignores note-off / CC release and sustained envelope ticks); no args (matches snapshot Play). Action GUID for snapshots is the companion row (`action.guid === snapshot.guid`).
- **Plugin `state` message** — `{ type: 'state', assignments, intents, actions, systemCapabilities, intentClasses }`. **`intents`** is `{ guid, name }[]` from **`GraphReplica.listIntentsForPlugin()`**. **`actions`** is `{ guid, name, executeType }[]` from **`listActionsForPlugin()`** (sorted snapshot → scene → animation → intent → other). **`systemCapabilities`** mirrors the hub **`register`** push (same YAML as surface input assign). **`intentClasses`** maps intent **`guid` → `class`** string from graph entities so the assign UI can build a single **dotKey** `<select>` from **`intentProperties[class]`** (respecting **`ignoreInParamsEditor`**); when descriptors are missing, the editor falls back to the legacy free-text dot path field.
...
- **Assign UI code** — ES modules under **`ui/js/`** (`assignApp.js`, session/list/modal, **`assignmentRegistry.js`**, **`targetHelpers.js`** / **`mountTargetEditor.js`**, **`viewers/`** per assignment class, **`components/learnFieldRow.js`**). Action-bound creates hide **`noteAndControl`** (continuous arm+CC). Add a new assignment class by implementing a viewer + registering it (mirror a new `Receiver*` in TypeScript).
...
### Hub-side resolution — `FixtureStateManager` + per-class resolvers (`fixtureState`)

The intent→fixture resolution (which the renderer `LayerIntentEngine` used to do) is moving **onto the hub** so renderers become thin output adapters. The hub resolves, **per fixture, one value per capability**, and streams the changed per-fixture `caps` as a `fixtureState` message; the renderer just composes for its medium. (The `target` lookAt intent was the pilot of this.)

- **`hub/src/resolve/FixtureStateManager.ts`** is the orchestrator: a ~50 Hz tick that, per fixture, dispatches to a **registry of per-class resolvers** and fills a `caps` object, then emits the changed fixtures (+ periodic resync for late joiners) **routed by zone** (`getZoneToRendererPayload`). It owns the per-fixture loop only — zero per-class branches; adding a new intent type (`audio`, `motor`, …) is a new resolver. Each tick it builds the effective intents (`getActiveSceneIntents` + `getEffectiveIntent`, class-normalized), computes each intent's zone by bbox containment, and caches fixtures from `getSerializedRuntimeZones` (rebuilt on `refresh`).
- **Resolvers** implement `resolve/CapabilityResolver.ts` (`intentClass`, `resolve(ctx, intents, caps)`, optional `onRefresh`): `resolvers/lightResolver.ts` (spatial color **blend** → `light.color.xyY` as `[x,y,Y]`, additive `light.strobe`/`light.brightness`, top-layer `light.aux`; uses `resolve/resolveColor.ts` blend math + `resolve/CapabilityResolver.computeSpatialFactor` — 1:1 ports of the renderer `LayerIntentEngine`), `resolvers/masterResolver.ts` (top-layer `master.brightness`/`master.blackout`), and `resolvers/targetResolver.ts` (the stateful one — eases a per-fixture aim direction from `target` magnets exactly as the former `PositionTargetManager`, contributing `caps.target = worldPos + easedDir`; a magnet's `params.mode` (`Attract` default / `Repel`) flips the contributed direction so `Repel` aims fixtures *away* with the same distance-weighted falloff). `light`/`master` zone-scope intents (positioned intent outside every zone reaches no fixture); `target` magnets are global by distance.
- **`fixtureState` message**: `{ type:'fixtureState', payload:[{ fixtureGuid, caps:{…} }] }`, one entry per changed fixture, sent only to renderers in that fixture's zone with `subscribe.fixtureState: true`.
- **Authored `target` intents** stay normal, visible intents on the `events` stream (e.g. `simulator-2d` draws the magnet via `EventPositionTarget`); they are only the resolver's input, not direct fixture drivers. A target's **height is `position[1]`** (metres; `position` is `[x, y, z]`) — relevant for moving heads aiming above the floor. The marker icon reflects `params.mode`: **Attract** draws four inward arrows in blue, **Repel** four outward arrows in orange (radius ring tinted to match). `simulator-2d`'s `EventPositionTarget` casts a floor **shadow** that drifts bottom-right and blurs as `position[1]` rises (constants under `EVENT_DRAW.shadow` in its `config.json`).
- **`surface-v2` height slider**: target intents get a thin vertical height slider drawn beside the cross in `viewport/overlayCanvas.js` (both edit and perform stage panes). It visualises/edits `position[1]` between the merged zone bbox y-range (`spatial.y1`..`spatial.y2`); the tiny track zooms ×`heightSliderEngagedScale` while touched (radial-knob style). Writes go through the existing `position` patch only (no new field): edit = durable (`projectGraph.updateIntentHeight` + save / scene overlay), perform = live `runtime:command` (`updateRuntimeIntentHeight`) — exact y-twins of the x/z drag path. Slider constants live in `surface-v2/config.json` LAYOUT (`heightSlider*`).
- **`surface-v2` fixture height**: fixtures reuse the same height-slider geometry, gated on `policy.canDragFixture` (i.e. Stage edit with fixtures unlocked). While unlocked each fixture gets the draggable slider (no floor shadow — fixtures are not rendered via `EventPositionTarget`); the drag calls `onFixtureHeightMove` → `projectGraph.updateFixtureHeight` and emits a durable `position` fixture patch via `queueFixtureUpdate` (y-twin of `onFixtureMove`). In perform, or when fixtures are locked, the slider is replaced by a small `"<y> m"` label beneath the fixture marker so the height stays readable without being editable.
- **`surface-v2` fixture instance editor**: double-tapping a fixture in Stage edit (same `policy.canDragFixture` gate as the height slider — edit + fixtures unlocked) opens **`FixtureParamsHost`** (`src/edit/fixture/FixtureParamsHost.js`), the fixture twin of `IntentParamsHost`, docked in the same `stage-edit` overlay region (the two are mutually exclusive). It edits the fixture **instance** layer only — the common root fields (`name`, `location` read-only, `range`) plus the per-instance **`params`** (`zones[].fixtures[].params`, e.g. `dmxBaseChannel`, `pan.trimDegrees`). The profile `params` (channel maps, `strobe`, `beam`) describe the physical model and are **not** edited here. The editable shape comes from a new root **`instance`** descriptor array in the fixture profile YAML (modeled on `systemCapabilities.intentProperties`), retained client-side from the `fixtureProfile` already shipped in `graph:init` (no extra hub message). The panel reuses the generic `PropertyPanel` / `PropertyControl` widgets via an injected **write target** (read/update/remove/save bound to `projectGraph.*FixtureProperty` + `queueFixturePropertyUpdate`); intents keep the default path unchanged. A `FixtureEditDefault` base (`src/edit/fixture/`) supplies generic behaviour with overridable lifecycle hooks (`rootDescriptors`, `paramDescriptors`, `validate`, `decorate`, `onSaved`); a fixture class that must interfere registers a subclass in `fixtureEditors/registry.js` (loaded lazily, falling back to the default). Keyed by fixture **guid** so it survives `name` edits. The overlay-canvas double-tap path (`setDoubleTapFixtureCallback`, routed through `stageOverlayCoordinator`) is wired only in edit mode.
- **Fixture moves rebuild hub resolver state**: a fixture `position` graph command sets `rendererConfigChangedFor` (all renderers). `index.ts` `publishGraphMutation` re-sends `config` to those renderers **and** calls `fixtureStateManager.refresh()` — the resolver caches each fixture's `worldPos` (e.g. `caps.target = worldPos + easedDir`), so without the refresh the hub keeps aiming against the pre-move position (renderers get a fresh config but wrong angles) until the next hub restart. **Non-position fixture patches** (instance `params`, `name`, `range`) are no longer stored as opaque entities: `ProjectGraphStore.applyFixtureCommand` routes them through `ProjectManager.patchFixtureByGuid` (dot-path apply into `zones[].fixtures[]` + persist), emits a fixture `upsert` delta carrying the full serialized instance (so controllers reconcile via `_applyFixtureDelta`), and sets `rendererConfigChangedFor` so renderers re-read changed params like `dmxBaseChannel`. **Fixture create** — an `upsert` for an unknown guid whose `value` carries a `position` (world) and a `fixture` (profile key) is routed to `ProjectManager.addFixture`: it resolves the target zone (by `parent` zone guid, else bounding-box containment of the world position), loads the profile via `Config` if unseen, stores a zone-relative `location`, dedupes the name within the zone, persists, and the delta `value` includes `zoneName` so `_applyFixtureDelta` inserts it into the right zone. **Fixture delete** — an `op:'remove'` is routed to `ProjectManager.removeFixtureByGuid` (splice from `zones[].fixtures[]` + persist), not the opaque store; the delta and `rendererConfigChangedFor` make renderers rebuild without it, and `_applyFixtureDelta` splices it from the controller replica. Both set `rendererConfigChangedFor` (all renderers) so `ConfigHandler` rebuilds the full fixture set.
- **Moving-head pan/tilt aim** (`dmx-ts` `dmx_moving_head_mini.applyAim`): each beam direction has two reachable poses — "front" (`pan = heading`, tilt below the up-centre) and "over the top" (pan held, tilt swung past vertical to the back: `pan = heading+180`). The renderer tracks the continuous world **front-heading** (`currentAimHeadingDeg`) plus pose flags `aimInCone` / `aimOverTop`. **Inside** a tight near-vertical cone (`OVER_TOP_CONE_DEG`, 2°) the front-heading is frozen and tilt alone carries the beam through vertical. **On cone exit** it may commit to over-top when that pose is nearer (so a real overhead crossing keeps pan still and uses the full tilt range). **Outside** the cone, over-top is only *kept* while `aimOverTop` is set — azimuth jumps alone never enter over-top (that previously flipped pan ~180° with tilt clamped after scene changes / sleep wake). Sleep-on-blackout clears the pose sticky state so wake re-seeds front. The front branch reduces to the original `90°−zenith` tilt and `atan2`+`panUnwrap` pan. `simulator-2d`'s moving head has no tilt channel (2D top-down icon), so it keeps the plain point-at-target icon rotation and does not mirror the over-top branch.

#### Renderer subscribe — `{ events, fixtureState }`
**All four renderers now consume `fixtureState`** (`subscribe.fixtureState: true`): their fixture classes read `snapshot.sample('light.color.xyY')`, `'master.brightness'`, `'target'`, … straight from the received caps (no local resolution), keeping only render-time work — strobe **blink gating** (`_isStrobeOn(nowSec)`), the moving head's `atan2`+`panUnwrap`, the DMX/BLE/canvas write. `dmx-ts`, `neewer`, `screen` are headless/output-only and `fixtureState`-only (their `LayerIntentEngine` copies are **deleted**). `simulator-2d` additionally keeps `subscribe.events: true` to draw intent **markers** (`EventLight`/`EventMaster`/`EventPositionTarget`) — its `LayerIntentEngine` is retained **only** as that marker store (its resolvers are unused; fixture output comes from caps). So the renderer-facing `events` stream now exists solely for the simulator's markers; the hub gates it via `wantsRendererEvents`.
...
Hub-owned runtime intent overlay state lives in **`RuntimeIntentStore`** (`hub/src/RuntimeIntentStore.ts`). Controllers send compact `runtime:command` patches; the hub accumulates them as a **patch bag** relative to the **live** active-scene intent row (definition + scene ref overlay), recomputes effective rows for wire payloads and renderer events, and normalizes through the **intent registry** (`hub/src/intents/`). Controllers do not perform this merge — they apply inbound `runtime:update` for sync only.
...
Policies are set on the overlay canvas via `overlay.setPolicy(policy)` when switching panes. The policy defines the drag-gate and move methods: `canDragIntent(intent)`, `canDragFixture(fixture)`, `onIntentMove` / `onIntentHeightMove`, `onFixtureMove` / `onFixtureHeightMove` (plus their `*End` twins). This keeps pane-specific interaction rules in a single testable object rather than scattered through the overlay code.
...
**Action** (`entityType: "action"`) — a named, executable graph entity stored at the top level of the project graph. Fields: `guid`, `name`, **`execute`** — a **single** object `{ type, guid, ... }`. Supported `type` values: `scene`, `intent`, `animation`, **`snapshot`**. Intent rows may include `params`, `patch`, `remove`, `value`, `scheduled` on `execute` (same shape as before, but not wrapped in an array).
...
Perform parameter payloads for intent (e.g. `argsOn` / `argsOff` / `args` as `jsonString` slots from `systemCapabilities.inputTypes`) are stored on the **action** under `execute.params` for `type: "intent"`, not on the input. **Intent `execute.params`** must use the reserved branch shape: either **`args`** (single plain object; keys are runtime patch dot paths) **or** **`argsOn` / `argsOff`** (plain objects; trigger `args` must include **`value`** to pick the branch). **Animation** companion actions (`action.guid` equals the animation `guid`): in **`runmode: "auto"`** the hub keeps **`execute`** as `{ type: "animation", guid }` (no `params`). In **`runmode: "manual"`** the hub ensures **`execute.params`** includes a **`command`** (defaults from `systemCapabilities.animations[].commands[0]` / animator registry, same as input-assignment and pulse bucket helpers) when the companion row would otherwise omit one; other action rows that target the same animation are unchanged. Controllers still merge **`action:trigger` `args`** with that action’s `execute.params`. Arbitrary flat keys on `execute.params` are not used for intent triggers.
...
- `removeInputAssignment` — scrubs every matching action GUID from inputs’ `actions[]`, then removes those actions **except** the **companion animation runner** (`action.guid` = animation `guid` and `execute.type` is `"animation"` with the same `guid`), which must stay so the animation remains triggerable and listed in Perform → Animate
...
Hub applies action changes via `graph:delta` and pushes **`projectPatch`** `{ key: "pulses", data: PulsesConfig }` to all controllers when buckets change. On controller register, hub sends an initial `pulses` patch after `graph:init`. Surface: `sendPulseAssignCommand(command)` in `outboundQueue.js`; `projectGraph` topic `pulses`. **surface-v2** (Perform → Animate): linking adds the bucket action via `pulse:assign`; the assign list’s **Edit** modal edits the bucket **name** (`renameBucket`) and, when linked, that row’s animation **`execute.params`** via `action:input` **`updateAction`** (same command/param UI as input-assigned animation actions) — not an extra `pulse:assign` payload.

**Snapshots** (project YAML `snapshots[]`): durable perform-state presets captured from live hub runners. Each row stores **`name`**, **`recall`** toggles (`scene`, `pulse`, `animations`), and a full payload always written at capture time: `activeSceneGuid`, `pulses[]` (`{ guid, speed }` per **running** pulse setup only), `animations[]` (`{ guid, timescale }` per **auto-mode** animation with active playback timers only — **manual** `runmode` runners are omitted; they stay trigger-driven via pulse/actions). Stopped/idle runners are omitted; there is no `isRunning` field — list membership implies auto playback was active at capture. Runtime intent overlays are **not** captured or recalled. Hub: [`SnapshotManager`](modules/hub/src/snapshot/SnapshotManager.ts) reads live state on **`snapshot:capture`**; recall via companion action `{ type: "snapshot", guid }` (`action.guid` equals snapshot `guid`, same pattern as animations). Recall order when toggles are on: scene (`runtimeMergeClear: 'all'`) → animations (selective stop/start, no `stopAll()`) → pulses (selective stop/start, no slot reset when already running). **`graph:command`** `entityType: "snapshot"` supports upsert/patch/remove; Modify **Save** patches **`name`** + **`recall`** only; Modify **Capture** sends **`snapshot:capture`** with the existing **`guid`** to replace the stored payload from live runners. Surface **Perform → Snapshot** pane (`SnapshotPane`, `performSnapshotPanel.js`): list, **+ Snapshot** modal (name + recall toggles), Modify/Delete, perform assign via `InputAssignManager` `context: { type: "snapshot", guid }`. Layout class `snapshot` is registered in `config.json` `LAYOUT_MANAGER`.

**Last-recalled highlight** (`snapshot:recalled`): when any controller recalls a snapshot, the hub broadcasts `{ message: { type: "snapshot:recalled", payload: { guid } } }` to **all** controllers (the recall's graph/runtime effects already fan out, but they don't carry the snapshot identity). `SnapshotManager` holds `lastRecalledSnapshotGuid` **in memory only** (never persisted to YAML); `RegisterHandler` replays it to each newly registered controller so fresh/reloaded surfaces show the same highlight. Controllers store it on `projectGraph` (`setLastRecalledSnapshot` / `getLastRecalledSnapshotGuid`, `snapshots` topic) and the Snapshot pane marks the matching row with `perform-snapshot-row--recalled` until the next recall. Purely a display marker — not stored on the snapshot, not part of `recall` toggles.

**`pulse:control`** (controller → hub): sends `PulseControlCommand` payloads. Commands include:
- `startSetup` — `{ setupGuid }`; starts (or restarts) that setup’s tick loop without stopping other running setups; persists `activePulseGuid` as the sync/tap focus target.
- `stopSetup` — `{ setupGuid }`; stops that setup only.
- `selectSetup` — `{ setupGuid }`; legacy alias for `startSetup`.
...
- `setSetupBpm` — `{ setupGuid, bpm }`; updates YAML; when that setup is running, hub refreshes that runner’s BPM from project (pending timer unchanged, per existing pulse policy)
- `setSetupSpeed` — `{ setupGuid, speed }`; clamps to `0.25`…`4`, stores on `pulses.setups[]`; tick spacing uses `bpm * speed` (incoming/sync/tap BPM × multiplier)
...
Hub pushes **`projectPatch`** `{ key: "pulses", data: PulsesConfig }` to all controllers on setup mutations. Surface Perform → **Pulse** subpane (`performPulsePanel.js`, `performPulseEditPane.js`): `sendPulseControlCommand(command)`; per-row **speed** (− / label / +); live slot meter from `hub:status` via `pulsePlayRegistry.js` (meter period uses `bpm * speed`).

**`pulse:tap`** (controller → hub): live tap-tempo stream (not durable per message). Payload `{ setupGuid, atMs? }`. Hub [`PulseTapTempo`](modules/hub/src/pulse/PulseTapTempo.ts) estimates interval from recent taps, smooths BPM, calls `PulseManager.setBPM`, broadcasts `hub:status`, and debounces `setSetupBpm` + `projectPatch`. If the setup is not running, `startSetup` runs first. Tunables: root-level **`pulse.tapTempo`** in hub `system.yml` (`minBpm`, `maxBpm`, `minTaps`, `windowMs`, `smoothing`, `persistDebounceMs`), merged into controller `systemCapabilities` as `pulse`.

**`pulse:sync`** (controller → hub): music-analyser (or similar) phase-aligned tempo sync. Payload `{ bpm, beatAtMs, sentAtMs, kind: "onset" | "bar", phaseAdjustMs?, audioT?, spectrum? }` — no `setupGuid`. On each valid inbound message the hub broadcasts **`hub:status`** `{ kind: "pulseSyncRx", data: { syncKind, atMs, bpm } }` to all controllers (`bpm` is the detected tempo from the inbound `pulse:sync` payload; surface Perform pulse toolbar blinks the **Sync** label orange and shows that BPM beside it — whether or not sync is enabled). Hub **ignores** tempo application when project **`pulses.sync.enabled`** is not `true`. When enabled, inbound **`bpm` &lt; 10** pauses all running pulse timers without stopping runners (no `hub:status` `stopped`; Perform play state unchanged). The next sync with **`bpm` ≥ 10** resumes with phase-aligned scheduling. [`PulseSync`](modules/hub/src/pulse/PulseSync.ts) lerps musical BPM **once** into a shared runtime value on [`PulseManager`](modules/hub/src/pulse/PulseManager.ts) (`syncSharedLiveBpm`), then distributes it to **every running** setup (`hub:status` `data.bpm` matches for all). Bar sync phase-aligns each runner independently using that shared BPM × per-setup **speed**; onset updates shared BPM without rescheduling. Disabling sync (`setSyncConfig` `enabled: false`) clears the shared live BPM. Hub estimates one-way latency from `sentAtMs`, computes the next beat boundary using the active setup’s **`speed`** with smoothed musical BPM (`periodMs = 60000 / (smoothedBpm * speed)`), smooths BPM toward the reported tempo using project **`pulses.sync.lerp`** (0–1; default `0.35`), clamps with tap-tempo `minBpm`/`maxBpm`, stores tempo on the runner as **`liveBpm`** (runtime only — does **not** call `setSetupBpm` or schedule a project save), and calls [`PulseManager.applyAlignedSync`](modules/hub/src/pulse/PulseManager.ts) (phase-aligned `setTimeout` + interval). Live BPM is shown via `hub:status`; durable `pulses.setups[].bpm` changes only through Perform pulse edit / `pulse:control` `setSetupBpm` (or tap-tempo persist). `kind: "bar"` always reschedules; `kind: "onset"` reschedules only when `|phaseAdjustMs| ≤ 50`. Optional project **`pulses.sync.restart`**: `never` (default), `bar`, or `onset` — when the incoming `kind` matches, hub sets `currentSlotIdx` to 0 before the next aligned tick (downbeat-style restart without an immediate fire). Project **`pulses.sync`** is durable YAML (preserved on load/normalize); it is not stripped when setups/buckets are normalized. `spectrum` is reserved for future FFT data.
...
Surface: reusable **`PulseTapButton`** (`edit/components/PulseTapButton.js`) in the perform **subnav** title bar (left of ☰ / tabs) and beside the BPM knob in the pulse edit pane; **Ctrl+T** fires a tap for the focused running setup (or first setup) via `sendPulseTap`. List rows show setup name + BPM (live BPM when running), play/stop toggle, and **speed** controls (− halves, + doubles, clamped `0.25`–`4`).

**`hub:status`** (`kind: 'pulse'`, hub → controllers): `{ setupGuid, status: 'started' | 'stopped', message: { text }, data: { bpm, slotIdx, slotsTotal, speed } }`. Emitted per setup on tick, start, stop; on controller register the hub sends one snapshot per running setup.
...
- **`AnimationManager`** — one runner per animation `guid`. Methods: `trigger(guid, { location, timescale })`, `stop(guid)`, `pause(guid)`, `setTimescale(guid, value)`, `enterEditMode(guid)`, `exitEditMode(guid)`. Requires at least one entry in `targetIntents` (or legacy singular target field) to start. Scene membership is true when **any** target intent is in the active scene. On status changes it broadcasts `hub:status` (`started` / `paused` / `stopped`) and emits `lock:intent` per target GUID so controllers disable those intents in UI.
- **`keyframeAnimator`** — the only runner class today. Requires `definition.content` with `content.length` and at least two steps (first/last times pinned to `0` and `length`, centisecond-rounded). Playback includes every step whose rounded time is within the rounded cycle length (so the last pinned keyframe is not dropped when raw float `length` and stored step times disagree in ms). Plays time-ordered keyframe steps against every GUID in `targetIntents` (same patch fan-out per step; lerp segments are planned per target from that target’s current effective state). Dispatches through `RuntimeUpdateDispatcher` (same path as a knob drag — no graph writes). When `content.lerp` is set, intermediate patches are planned by `paramLerpSchedule.planIntermediateLerpPatches()` using the named function curve and the quantization/minMs/time settings; each auto segment is queued **after** the previous anchor’s keyframe has been applied so the ramp starts from that last-applied state toward the next anchor. In manual mode, the same planner runs over `lerp.time` (wall-scaled by timescale) for `step` / `goto` / `random`, from the last **committed** keyframe index to the requested target (so e.g. `random` does not assume a sequential predecessor). An animation may have zero targets (it remains in the project graph but does not drive intents until targets are assigned).
...
- **Pulse panel** (`performPulsePanel.js`) — pulse setup list (name + BPM + speed) with play/stop/create/edit; multiple setups may run concurrently; slot meter from `hub:status` (`kind: 'pulse'`); subscribes to `pulsePlayRegistry`.
...
### Connector physics (`PhysicsEngine`)

Intents (any class **except `master`**) can be physically linked so moving one drags its neighbours. The solver runs on the hub under `hub/src/physics/` and is split into a pure engine and an intent-aware consumer:

- **`PhysicsEngine`** — generic point-mass solver. Knows only bodies (`{ position, velocity, mass, drag, pinned }`) and connectors; nothing about intents, zones, or transport. It is **time-constant**: a fixed sub-step accumulator (`advance(frameMs)`) advances the same simulated time per wall-second whatever the wall tick rate (default sim rate 20 Hz), so results never depend on render fps. It runs a recursive `setTimeout` loop while awake and **sleeps to 0 Hz** once every free body's speed is below `sleepVelocity`; `wake()` restarts it. Each sub-step is **semi-implicit Euler + PBD**: (1) damp `velocity *= (1−drag)^dt` — `drag:0` keeps momentum forever, so an undamped network never sleeps by design; (2) soft connectors add force impulses to velocity; (3) `position += velocity·dt`; (4) rigid connectors project positions; (5) `velocity = (position−prevPosition)/dt` so rigid corrections also act as impulses (momentum carries between connected bodies). Config from `system.yml → physics` (`fps`, `sleepVelocity`, `iterations`).
- **Connectors** (`physics/connectors/`, base + registry like `EventBase`): **Spring** — a Hooke force `F = k·(dist−restLength)` (k from `springForce` 0.1..0.9) applied via `applyForce`, so bodies accelerate/overshoot/oscillate; damping comes only from body `drag`. **Rod** — rigid distance constraint via `project` (`rotationSpring` 0..1 blends keeping the original orientation). **Rope** — `project` only when stretched past `restLength`. Corrections split by **inverse mass** (heavier moves less; pinned immovable).
- **`PhysicsIntentAdapter`** — the consumer. Builds bodies from active-scene intents (`mass`/`drag`/`position`, skipping `master`) and connectors from `ProjectManager.getActiveConnectors()` (scene-scoped, see below); clamps each committed position to the stage AABB in the engine's commit hook (a clamped axis zeroes that velocity component — collision stays out of the engine); enforces **per-axis freeze** in the same commit hook (from the intent's `freeze: [x,y,z]` boolean tuple, cached at rebuild as `{ freeze, lockPosition }`; a frozen axis is restored to its locked value and its velocity zeroed each tick, so an intent can be pinned on selected axes while still moving on others); and streams results as transient `runtime:update`s (`source: 'hub:physics'`), which `FixtureStateManager`'s tick then picks up.
- **Dragging via a temporary drag link.** Perform dragging carries an explicit lifecycle on the `runtime:command`: `drag:'move'` while held, `drag:'end'` on pointer-up. The adapter registers a `RuntimeUpdateDispatcher` *interceptor* that **claims** these (and consumes them, so the raw position never reaches renderers): a `'move'` creates/holds a transient **fixed anchor** body (pinned) plus a transient **`DragConnector`** and redirects the cursor position into the anchor; the engine then owns the dragged intent's rendered position. The `DragConnector` is a **critically-damped** point follow (no spring bounce) whose force is **clamped to `maxForce`**, so a fast/far cursor move cannot inject a huge impulse that whips down the connection chain — the key stability property. The intent's `mass` governs how it lags the cursor, connected intents follow through bounded forces. Release is **explicit** — `drag:'end'` (pointer-up) drops the anchor + link immediately (never on a timeout, never while held), and the intent flies on with its momentum. Edit-mode placement, animations and other updates are unmarked, so they pass through as direct position. Tuning: `physics.dragStiffness`, `physics.dragMaxForce`.

**Connector entity & editing.** A connection is a durable graph entity `connector` (`{ guid, kind, aGuid, bGuid, restLength, params, inScenes? }`), referenced symmetrically by both intents — single source of truth, no mirroring. It is stored via the opaque-entity path (`ProjectGraphStore.applyConnectorCommand` → `upsert/patch/remove`) and surfaces in `entities.connector` for controllers and in the renderer `config` payload (`connectors`) for the simulator. Removing either endpoint intent **cascades** the connector removal (`ProjectGraphStore.removeConnectorsReferencing`), so no stale link survives. surface-v2 edits links in the intent params overlay (`ConnectionsEditor`): list = connectors where `aGuid|bGuid` is the selected intent, Connect = `upsert`, kind/param = `patch`, trash = `remove`. Kinds/params are declared in `systemCapabilities.connectorTypes`. Connector **visuals are simulator-2d only** (`renderers/simulator-2d/src/connectors/` — rod=line, spring=zigzag, rope=sagging 5-point `RopeChain`); the motion itself is ordinary intent position updates every renderer already consumes, so `dmx-ts` needs no change.

**Scene scoping (`inScenes`).** A connector's optional `inScenes` array scopes it to specific scenes: empty/absent means active in **every** scene (default); a non-empty list means active **only** when `project.activeSceneGuid` is in the list. The hub is the single enforcement point — `ProjectManager.getActiveConnectors()` filters by the active scene, and **both** consumers use it: `buildRendererConfig` (so the simulator only draws active connectors) and `PhysicsIntentAdapter.buildConnectors` (so suppressed connectors exert no force). `getConnectors()` stays unfiltered (the durable list). Switching scenes re-pushes renderer config (`activateScene` now sets `rendererConfigChangedFor` to all renderers) and rebuilds physics (`index.ts` scene-activate resync), so scope changes take effect live. surface-v2's `ConnectionsEditor` row shows a scene button (label = scene name(s), or "All" when unscoped) opening `connectorScenesPicker` (multi-select modal with an "All"/"One" toggle); OK persists `inScenes: []` when every scene is selected (canonical "all") via a `connector` `patch`.

**Rules:**
- Physics mutations are transient — emit via `RuntimeUpdateDispatcher` only, never write durable graph state from the loop.
- `mass` (0.1–10, quadratic) and `drag` (0–1) are intent properties on spatial classes (`light`, `target`) in `system.yml → intentProperties`; never on `master`.
...
- `TargetIntent.transformToNormalized` is a pass-through — `target` (lookAt magnet) intents carry only geometry; their per-fixture effect is resolved by the hub `targetResolver` under `FixtureStateManager` (see *Hub-side resolution* above), not shaped here.
...
- `connectorTypes[]` — `{ kind, name, params: [{ dotKey, name, range, step, defaultValue }] }` for the physical link kinds (`rod`/`spring`/`rope`). Drives the surface-v2 `ConnectionsEditor`; read via `getConnectorTypes()`. See **Connector physics**.
...
`surface-v2/src/core/help/` is a **reusable, domain-free** in-app help subsystem: a floating/host-attachable help panel plus a persistent draggable ❓ open button. It takes **zero domain imports** — it knows nothing about intents, the hub, or the layout shell. Callers couple it to the app through two injection points wired in [`src/app/main.js`](modules/controllers/surface-v2/src/app/main.js): a **host registry** and a **conduit** (see also the controller-side reuse rule under [Obligatory Guidance For Coding Agents](#obligatory-guidance-for-coding-agents)).

- **`HelpManager.js`** — the singleton. Public API `{ registerHost, show, hide, setConduit }`. `show(key, { host?, onClose?, force?, quiet? })` looks up a topic and renders it (`quiet: true` makes an unknown key a silent no-op instead of falling back to `no-help-available` — used by the `data-help` binder so a typo'd attribute never pops a card); with `host` it attaches into a registered host element as a full-host card, otherwise it floats (draggable + resizable, geometry persisted). **Open vs. automatic-help are separate concerns:** a persistent draggable **❓ open button** (`.help-open-icon`) sits in the DOM whenever no panel is shown — clicking it **always opens** the index topic (`show('index', { force: true })`) regardless of the toggle, and it dims to reflect the toggle state. The **automatic-help toggle** (`.help-toggle`, "Auto: on/off") lives **inside the open panel's header** left of the `×`; flipping it persists global help visibility but **never closes the panel** — it only governs whether future programmatic help appears. While a panel is open the floating open button is hidden; on close it reappears. **Non-mandatory** topics are silently skipped when automatic help is toggled off (unless `force`) and are dismissable via the `×` button or **Esc** (which fires the caller's `onClose`); **mandatory** topics (e.g. an interaction prompt like `connectIntent`) always show and cannot be Esc/×-dismissed. A `generation` counter guards against an in-flight async `show` being superseded; `hide()` is idempotent and re-entry safe.
- **`loadHelpContent.js`** — fetches and memoizes `help.json` (a flat dictionary `key → { heading, text, mandatory? }`). Domain-free: load failures are `console.warn` only, never surfaced through app status. Unknown topic keys fall back to the `no-help-available` topic.
- **`helpPanelState.js`** — localStorage persistence of the single shared panel geometry (`ambitecture.surface-v2.helpPanel`) and the visibility flag (`ambitecture.surface-v2.helpVisible`). One geometry is reused by every floating topic.
- **`renderHelpText.js`** — renders a topic's raw text into safe DOM nodes via a small inline markup: `\n` → `<br>`, `[text](url)` links, and `${displayPlugin:fnName(args)}` placeholders. It never uses `innerHTML`. A display plugin's `conduit.callFunction()` may return either data **or a Promise**: for a Promise the renderer drops in a `help-display-loading` placeholder and swaps it for the rendered result when it resolves (so a placeholder can show live hub data fetched over the wire) — this stays generic, the async source is the caller's concern. Re-exports `registerProtocol` / `registerDisplayPlugin` for extension.
- **`protocols/`** — link-handler registry keyed by URL scheme. `http`/`https` (`http.js`) render an external `<a target="_blank" rel="noopener noreferrer">`; a URL with no `://` is classified as **`internal`** (`internal.js`) — a bare topic key rendered as a button that calls `ctx.showTopic(key)` to navigate within the panel. Add new schemes with `registerProtocol(scheme, handler)`.
- **`display/`** — display-plugin registry for `${plugin:fn(args)}` placeholders. The built-in **`listView`** (`display/listView.js`, registered as a side-effect import) renders a `{ name, entity }[]` as a list of internal-topic links (`entity.name`). Add new plugins with `registerDisplayPlugin(name, fn)`.
- **`conduit`** — the data bridge a `${plugin:fn()}` placeholder calls. `HelpManager.setConduit({ callFunction(name, args) })` is wired by the host so display plugins can pull **live app data** without the help system importing anything domain-specific. The surface-v2 implementation is the **`HelpConduit`** class ([`src/app/HelpConduit.js`](modules/controllers/surface-v2/src/app/HelpConduit.js)) — app-layer glue, not inside `help/` — with one `callFunction` case per help function. Most cases return a Promise from the hub **`system:probe`** endpoint (e.g. `getRendererList` → `hubProbe.probe('connectedRenderers')`, then maps each `{ type, guid }` to the `listView` shape `{ entity: type, name: `${type} (${guid})` }`) so the panel shows real connected modules instead of hardcoded data.
- **`hosts`** — `HelpManager.registerHost(name, elementOrGetter)` lets callers pass `show(key, { host: 'edit-panel' })` without the manager importing layout/DOM helpers; the getter resolves the target element lazily.
- **`data-help` wiring** — buttons/controls opt into contextual help declaratively with a `data-help="<topicKey>"` attribute. A single delegated listener in the **app layer** ([`src/app/attachDataHelp.js`](modules/controllers/surface-v2/src/app/attachDataHelp.js), wired once in `main.js`) reads `closest('[data-help]')` on click and calls `show(key, { quiet: true })`. The reusable core knows nothing about `data-help` — the attribute convention is an application decision. Non-DOM gestures (canvas double-tap, etc.) that have no element to tag call `showHelp(key)` from [`src/app/actionHelp.js`](modules/controllers/surface-v2/src/app/actionHelp.js) instead. Topic content for the stage/edit/perform/scene/intent areas lives in [`help.json`](modules/controllers/surface-v2/help.json) under a dotted namespace (`stage.*`, `intent.edit.*`, `scene.*`, `snapshot.*`, `connections.*`).

**Rules:**
- Keep `src/core/help/` domain-free. Do not add hub/intent/layout imports. Surface live data through the **conduit** and target elements through the **host registry**, both wired by the caller in `main.js`.
- Author help copy as topics in [`help.json`](modules/controllers/surface-v2/help.json), not as strings in code. Link between topics with bare-key `[text](topicKey)` links; pull live data with `${displayPlugin:fn(args)}`.
- Extend rendering through the protocol and display-plugin registries — never special-case schemes or placeholder names inside `renderHelpText.js`.
- Use `mandatory: true` only for prompts that must not be dismissed by toggle/Esc (interaction guidance); everything else stays non-mandatory.
- Tag buttons with `data-help` (markup, in the app/product layer); reserve inline `showHelp`/`show` calls for non-DOM gestures. To author or extend help for a UI area, follow the `/help-authoring` skill.
...
**`params.power.sleepOnBlackout`** (boolean) lets a fixture "sleep" while fully dark. The effective value resolves per fixture as the **instance** override (`params.power.sleepOnBlackout`, set by the surface-v2 fixture editor) falling back to the **profile** default (`fixtureProfile.params.power.sleepOnBlackout`), else `false`. When enabled **and** the fixture's resolved output is `0`, renderers stop driving the fixture's sleepable hardware: the DMX moving head (`dmx_moving_head_mini`) and derby (`derby_3c`) skip pan/(tilt)/`xy-speed` writes so the motors rest in place (brightness still goes to 0); the `neewer` and `screen` renderers skip the BLE send / pixel refresh, using a per-fixture **settled-dark latch** so the device still receives the final 0 (turns off) on the transition frame before sleeping. `simulator-2d` **intentionally ignores** `sleepOnBlackout` — it keeps tracking the target so the operator can see where a sleeping head would aim once it wakes. Authored as an `instance` descriptor (see below), the editor exposes it only on profiles that opt in (currently `moving_head_mini.yml`, `derby_3c.yml`).

A channel under `params.channels` may carry a **`function`** describing hardware output response. The driving renderer applies it on output to linearise perception. This is **hardware compensation only**: `simulator-2d` intentionally does **not** apply these curves, since after compensation the physical device should match the simulator's intended-perceived output. When a channel has no `function`, output is passed through linearly (no implicit default).
...
At startup the hub resolves the project in this order: explicit CLI argument or `HUB_PROJECT` env (from repo `./start.sh <name>` / PM2), else the persisted specifier in `var/hub/activeProject.json` (`ActiveProjectStore` via `FsStorage` under `system.yml` `dataDir`). There is no implicit default project — if neither an explicit specifier nor a persisted `activeProject` exists, startup throws and the process exits. When an explicit project is chosen and loads successfully, the hub writes that specifier back to `activeProject.json`. If the chosen specifier (explicit or persisted) does not resolve to an existing project YAML file, startup throws and the process exits. Pass a project as the first CLI argument after the entry script (for example `npm run dev test2` from `modules/hub`, which forwards the name to Node). The value is either a basename under `projectsPath` (`.yml` added) or a path to a `.yml` / `.yaml` file; see `ProjectManager.resolveProjectYamlPath`.
...
Manages pulse setups and runtime control. Payload is a `PulseControlCommand` (`startSetup`, `stopSetup`, `selectSetup`, `createSetup`, `deleteSetup`, `renameSetup`, `setSetupBpm`, `setSetupSpeed`, `setSetupSlotCount`, `setSetupMode`, `assignSlotBucket`, `setSlotActive`, `setSyncConfig`). Setup changes are pushed as `projectPatch` key `pulses`; `startSetup` / `stopSetup` drive per-setup `PulseManager` tick loops and `hub:status` (`kind: 'pulse'`) updates. Only slots with `active: true` dispatch bucket actions on tick.
...
- `light.strobe` accumulates in ascending layer order with the same spatial falloff as color (fixture range curve × intent radius curve when `radius` is set); intent **`alpha` does not scale strobe**—`alpha` applies only to color mixing. The `neewer` renderer (intent `light.strobe` / `light.aux.strobe`, fixture profile **`params.strobe`** with `lowFrequency`/`highFrequency`/`onTime`) has no strobe channel and is edge-triggered, so it simulates strobe with a per-fixture self-rescheduling timer (`StrobeScheduler`, owned by the shared `strobeRegistry`): the "on" beat sends the lamp's target brightness, the "off" beat sends brightness 0, period = `1 / (lowFrequency + strobeValue·(highFrequency−lowFrequency))`. The on flash lasts `onTime` but is capped at half the period, so a short period (high frequency) still leaves an equal off phase instead of staying lit. When the target brightness is already 0 (or strobe is 0) it sends a single steady write instead of flashing, keeping the BLE bus quiet; `strobeRegistry.stopAll()` runs on every config reload so timers re-arm with the new config.
