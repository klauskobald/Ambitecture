...
- **Moving-head pan/tilt aim** (`dmx-ts` `dmx_moving_head_mini.applyAim`): each beam direction has two reachable poses — "front" (`pan = heading`, tilt below the up-centre) and "over the top" (pan held, tilt swung past vertical to the back: `pan = heading+180`). The renderer tracks the head's continuous world **front-heading** (`currentAimHeadingDeg`); **outside** a tight near-vertical cone (`OVER_TOP_CONE_DEG`, 2°) it snaps that heading to whichever of `heading`/`heading+180` is closer (so a beam that went over the top stays there instead of spinning pan 180°), and **inside** the cone it freezes the heading while the in-plane tilt formula (`atan2(dy, horizontalDist·cos(rel))`) carries the beam smoothly through vertical and out the far side — using the full tilt range. The front branch reduces to the original `90°−zenith` tilt and `atan2`+`panUnwrap` pan. `simulator-2d`'s moving head has no tilt channel (2D top-down icon), so it keeps the plain point-at-target icon rotation and does not mirror the over-top branch.
