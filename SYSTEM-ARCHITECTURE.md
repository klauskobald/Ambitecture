...
- **Authored `target` intents are normal intents** — placed/edited like a `light` (`position`, `layer`, `radius`, `radiusFunction`, `params.alpha`, `params.speed`). They are emitted to renderers as ordinary intent events so the magnet is **visible** (e.g. `simulator-2d` draws it via `EventPositionTarget`, an influence-radius ring + crosshair). They do **not** drive any fixture directly — no fixture samples a `target` capability — they are purely the resolver's input. A live drag flows as usual (runtime overlay → event), which both moves the marker and feeds the resolver.
- **`PositionTargetManager`** (`hub/src/targets/PositionTargetManager.ts`, modeled on `AnimationManager`) runs a ~50 Hz tick. Followers = fixtures whose profile DMX map declares a `pan` function; it reads their world position (`zone.bbox + location`), per-instance `params.maxFollowTime`, and `guid` from `getSerializedRuntimeZones()`. Active magnets = active-scene `target` intents (live via `getEffectiveIntent`). Per follower: `strength = alpha × radiusFn(1 − dist/radius)`; only the **highest in-range layer** contributes (layer override); the aim point is the strength-weighted mean of those magnets, eased on the fixture's own clock over `min(params.speed, maxFollowTime) / strength` seconds (`params.speed` is the authored follow time in seconds; `0` = instant snap; weaker magnet = slower; no magnet in range = hold). The speed slider authors seconds directly with a `quadratic` `stepFunction` (fine control near 0) and a max of `RUNTIME:maxFollowTime` (largest per-fixture `maxFollowTime` in the project, via `resolvers/MaxFollowTimeResolver.ts`), so its label reads the resulting time and runs `0`→max.
