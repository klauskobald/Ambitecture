...
- **Authored `target` intents** stay normal, visible intents on the `events` stream (e.g. `simulator-2d` draws the magnet via `EventPositionTarget`); they are only the resolver's input, not direct fixture drivers.
