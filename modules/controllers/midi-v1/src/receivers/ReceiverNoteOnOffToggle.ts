...
interface NoteOnOffToggleParams {
  note: number;
  velocityMin: number;
  velocityMax: number;
...
function readParams(raw: Record<string, unknown>): NoteOnOffToggleParams | null {
  const note = raw['note'];
...
export class ReceiverNoteOnOffToggle extends ReceiverBase {
  ...
  constructor(
    ...
    targets: TargetBase[],
    logger: Logger,
    private readonly params: NoteOnOffToggleParams,
  ...
  private ensureEnvelope(note: number): EnvAr {
    ...
    if (!ep?.enabled) {
      throw new Error('ReceiverNoteOnOffToggle.ensureEnvelope without enabled envelope');
    }
    if (!env) {
      env = new EnvAr({
  ...
  private applyOn(e: MidiNoteEvent): void {
  ...
  private applyOff(): void {
    ...
    this.signalAssignmentActivity();
    const env = this.envelopes.get(this.params.note);
    if (env) env.noteOff();
    else this.fanOut(0);
  }
