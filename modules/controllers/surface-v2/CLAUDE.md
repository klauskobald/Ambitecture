...
- **2026-06:** Help system (`src/core/help/`): reusable, **domain-free** in-app help — floating/host-attachable panel + persistent ❓ open button. `HelpManager.show(key, { host?, onClose?, force? })`; topics authored in `help.json` (`{ heading, text, mandatory? }`). Open and automatic-help are split: the floating **❓ button always opens** help (`force`); the **"Auto: on/off" toggle** lives in the panel header (left of ×) and only governs whether programmatic non-mandatory help shows — `mandatory:true` topics always show. Coupled to the app only via `registerHost(name, getter)` and `setConduit({ callFunction })`, both wired in `main.js`. Inline markup in `renderHelpText.js`: `[text](topicKey)` internal links, `http(s)://` external links, `${plugin:fn(args)}` placeholders. Extend via the protocol registry (`protocols/`) and display-plugin registry (`display/`, e.g. `listView`) — never add domain imports or special-cases inside `help/`. See SYSTEM-ARCHITECTURE.md → Help system.
...
- **2026-06:** Help content + `data-help` wiring. Stage/edit/perform/scene/intent/snapshot/connections help authored in `help.json` under a dotted namespace (`stage.*`, `intent.edit.*`, `scene.*`, …). Buttons opt into contextual help **declaratively** with a `data-help="<topicKey>"` attribute; a single delegated listener in the app layer (`src/app/attachDataHelp.js`, wired in `main.js`) calls `HelpManager.show(key, { quiet: true })` on click — `quiet` makes an unauthored key a silent no-op. Non-DOM gestures use `showHelp(key)` from `src/app/actionHelp.js`. The `data-help` convention is app-layer only; `src/core/help/` stays domain-free (gained just the generic `{ quiet }` option). **To author/extend help for any UI area, run the `/help-authoring` skill** (`.claude/skills/help-authoring/SKILL.md`).
