...
import { createButtonForInput } from './performButtons/performButtonFactory.js'
...
class KeyboardManager {
  ...
  _onKeyDown (event) {
    ...
    if (
      ...
      !actionGuids.every(ag => projectGraph.getActions().has(ag))
    )
      return
    ...
    if (behavior === 'momentarySwitch') {
      ...
      performMomentaryPress(inputGuid, `kbd:${keyChar}`, actionGuids)
      return
    }
    ...
    if (behavior === 'toggle') {
      ...
      const performButton = createButtonForInput(
        inputGuid,
        /** @type {Record<string, unknown>} */ (input),
        stub
      )
