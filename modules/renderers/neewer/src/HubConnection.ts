...
export class HubConnection {
    ...
    private readonly bleBus: NobleBleBus;
    private readonly discovery: DiscoveryService;
    private readonly neewerBus: NeewerBus;

    constructor() {
        this.bleBus = new NobleBleBus();
        this.discovery = new DiscoveryService(this.bleBus, Config.scanRestartMs);
    ...
    async start(): Promise<void> {
        try {
            ...
            Logger.error('[ble] discovery failed to start', err);
        }
        this.connect();
    }
    ...
    connect(): void {
        ...
        ws.on('close', () => {
            if (gen === this.connectGeneration) this.clearConnectTimeout();
            if (gen !== this.connectGeneration) return;
