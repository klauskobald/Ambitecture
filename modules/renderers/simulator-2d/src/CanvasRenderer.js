class CanvasRenderer {
  constructor (canvasEl, bootConfig) {
    ...
    this.ppm =
      typeof ppmRaw === 'number' && Number.isFinite(ppmRaw) && ppmRaw > 0
        ? ppmRaw
        : 50
  ...
  setSpatialFromZones (zones) {
    ...
    const validZones = zones.filter(
      z =>
        z &&
        typeof z === 'object' &&
        Array.isArray(z.boundingBox) &&
        z.boundingBox.length >= 6
    )
    if (validZones.length === 0) {
      console.warn(
        '[CanvasRenderer] setSpatialFromZones: no boundingBox on zones'
      )
      return
    }
    this._zones = validZones.map(z => ({
      name: z.name,
      bbox: z.boundingBox.map(Number)
    }))
